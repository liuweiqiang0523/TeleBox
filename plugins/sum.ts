...
const modePrompts: Record<Exclude<SumMode, "summary" | "person">, string> = {
  ...
  cp:
    "你是 Telegram 群聊互动嗑糖榜助手。请只分析群聊里的接话、互相回应、互相点名和默契配合；这是纯节目效果，不暗示现实关系。\n\n【版式要求】\n1. 固定使用下面模板，不要增删一级栏目。\n2. 禁止使用 **Markdown 加粗**，标题会自动加粗。\n3. 禁止使用单独起行的“｜”；不要把字段拆成半句话。\n4. 只写群聊互动，不写暧昧、性暗示、现实关系推断。\n5. 必须写互动次数，优先使用“本地互动候选”里的“连续互动约 N 次”；如果是点名互动，写“点名约 N 次”。\n6. 副榜数量按“输出控制”执行：消息少不要硬凑，互动低于 3 次的组合只有特别有名场面时才写。\n7. 每组必须有 4-8 字称号，例如“脚本连麦”“技术搭子”“复读默契”“补刀拍档”“冲机拍档”。\n8. 最甜 CP 只写 1 组，字段必须整齐：主角、称号、互动、糖点、名场面、糖度。\n9. 其他组合用编号卡片式排版，每组 2 行；组与组之间空一行。\n10. 名场面必须来自输入原文或非常接近原文；没有合适原话就省略名场面字段。\n11. 没有明显互动组合就写“无明显组合”。\n\n【输出模板】\n# 🍬 今日互动嗑糖榜｜群名\n\n## 🥇 最甜 CP\n👥 主角：用户A ↔ 用户B\n🏷️ 称号：甜味称号\n🔁 互动：连续互动约 N 次\n🍯 糖点：用户A负责什么，用户B负责什么，两人像什么场景。\n🎬 名场面：用户A「短原话」 / 用户B「短原话」\n💞 糖度：★★★★★\n\n## 🧃 其他组合\n1. 用户C ↔ 用户D\n   🏷️ 称号：短称号｜🔁 约 N 次｜💞 ★★★★☆\n   🍯 糖点：一句话说明默契在哪里。\n\n2. 用户E ↔ 用户F\n   🏷️ 称号：短称号｜🔁 约 N 次｜💞 ★★★☆☆\n   🍯 糖点：一句话说明默契在哪里。\n\n3. 用户G ↔ 用户H\n   🏷️ 称号：短称号｜🔁 约 N 次｜💞 ★★★☆☆\n   🍯 糖点：一句话说明默契在哪里。\n\n## 🍰 今日糖型\n- 技术连麦：一句话说明谁和谁最像这种类型\n- 补刀接梗：一句话说明谁和谁最像这种类型\n\n## 🧭 今日结论\n一句话说明今天最有戏的一组，控制在 45 个中文字符内。",
  abstract:
...
function prepareCpInput(records: ChatMessageRecord[]): PreparedInput {
  ...
  const outputHint = records.length >= 800
    ? "输出控制：最甜 CP 1 组；其他组合 6-8 组；每组必须写互动次数、称号和糖度；不要照抄全部候选。"
    : records.length >= 200
    ? "输出控制：最甜 CP 1 组；其他组合 4-6 组；每组必须写互动次数、称号和糖度；不要照抄全部候选。"
    : "输出控制：最甜 CP 1 组；其他组合 3-5 组；不要硬凑低互动组合；互动少于 3 次的组合只有特别有名场面时才写。";
